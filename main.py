...
from ldm.data import joinaudiodataset_anylen, joinaudiodataset_struct_sample_anylen
...
def nondefault_trainer_args(opt):
    ...
    parser = Trainer.add_argparse_args(parser)  # torch2
...
class DataModuleFromConfig(pl.LightningDataModule):# batchloader outputshape should be (b,h,w,c) and it will be permuted to (b,c,h,w) in autoencoder.get_input()
    ...
    def _train_dataloader(self):
        init_fn = None
        if isinstance(self.datasets["train"], joinaudiodataset_anylen.JoinManifestSpecs):
        ...
        elif isinstance(self.datasets["train"], joinaudiodataset_struct_sample_anylen.JoinManifestSpecs):
    ...
    def _val_dataloader(self, shuffle=False):
        init_fn = None
        if isinstance(self.datasets["validation"], joinaudiodataset_anylen.JoinManifestSpecs):
            from ldm.data.joinaudiodataset_anylen import DDPIndexBatchSampler
            ...
            return DataLoader(dataset, batch_sampler=batch_sampler,sampler=None,
                          num_workers=self.num_workers, collate_fn=dataset.collater,
                          worker_init_fn=init_fn)
        elif isinstance(self.datasets["validation"], joinaudiodataset_struct_sample_anylen.JoinManifestSpecs):
            from ldm.data.joinaudiodataset_struct_sample_anylen import DDPIndexBatchSampler
            dataset = self.datasets["validation"]
            main_indices = dataset.ordered_indices()
            # main_indices,other_indices = dataset.ordered_indices()
...
if __name__ == "__main__":
    ...
    try:
        ...
        if 'ignore_keys_callback' in callbacks_cfg and hasattr(trainer_opt, 'resume_from_checkpoint'):# false for the former
